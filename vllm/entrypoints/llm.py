...
class LLM:
    ...
    def train(
        ...
        *,
        ...
        if lora_request is not None and any([lora_path, lora_name, lora_int_id]):
            raise ValueError(
        ...
        for req_id, example in zip(request_ids, training_data):
            ...
            training_loss = output.training_loss if output and hasattr(output, 'training_loss') else None
            ...
            if 'text' in example:
                ...
                num_tokens = len(example['labels']) if isinstance(example['labels'], list) else len(example['labels'])
        ...
        for i, example in enumerate(it):
            ...
            # Format 3: Legacy 'prompt' + 'labels' format
                ...
                if 'prompt' in example:
                    ...
                    prompt_token_ids = tokenizer.encode(prompt) if isinstance(prompt, str) else None
                else:
